...
import { deleteProject, unpublishProject, type Project, type DashboardUser } from "@/lib/actions/projects";
...
export default function DashboardClient({ initialProjects, dashboardUser }: Props) {
  ...
  const { whop, internal: user } = dashboardUser;
  ...
  const handleUnpublish = async (id: string) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        setUnpublishError(result.error || "Failed to unpublish project");
        return;
      }

      setProjects(projects.map((p) =>
        p.id === id ? { ...p, isPublished: "false", liveUrl: null } : p
      ));
              ...
              >
                ...
                <div className="relative h-full p-5 flex flex-col">
                  ...
                  <div className="absolute inset-x-5 bottom-5 flex items-center gap-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                    ...
                    {project.isPublished === "true" && (
                      <button
                        onClick={() => setUnpublishConfirm(project.id)}
                        className="p-2.5 rounded-xl bg-white/10 hover:bg-amber-500/20 hover:text-amber-400 transition-colors"
                        title="Unpublish"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="relative w-full max-w-sm rounded-2xl bg-[#141416] border border-white/10 shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setDeleteConfirm(null)}
                  disabled={isPending}
                  className="flex-1 py-2.5 rounded-xl bg-white/5 text-white/70 text-sm font-medium hover:bg-white/10 transition-colors disabled:opacity-50"
                >
                  Cancel
                </button>
                <button
        ...
        {unpublishConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-sm rounded-2xl bg-[#141416] border border-white/10 shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
              ...
              <h3 className="font-['Sora',sans-serif] font-semibold text-lg text-center mb-2">Unpublish Project?</h3>
              <p className="text-white/40 text-center text-sm mb-6">
                Your site will be taken offline and the live URL will stop working. You can republish it anytime.
              </p>
              {unpublishError && (
                <div className="mb-4 px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 text-sm text-center">
                  {unpublishError}
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => handleUnpublish(unpublishConfirm)}
                  disabled={isPending}
                  className="flex-1 py-2.5 rounded-xl bg-amber-500 text-white text-sm font-medium hover:bg-amber-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {isPending ? (
                    <>
                      <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
